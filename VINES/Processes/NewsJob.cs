    ...
    public class NewsJob : IJob
    {
        ...
        public void getPNA()
        {
            try
            {
                ...
                var articleList0 = htmlDocument.DocumentNode.Descendants("div")
                    .Where(node => node.GetAttributeValue("class", "")
                    .Equals("opened")).ToList();
                var articleList1 = articleList0[0].Descendants("div")
                    .Where(node => node.GetAttributeValue("class", "")
                    .Equals("folder")).ToList();
                var articleList = articleList1[0].Descendants("div")
                    .Where(node => node.GetAttributeValue("class", "")
                    .Equals("opened")).ToList();
